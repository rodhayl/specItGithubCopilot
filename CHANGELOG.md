...
### Added
- **GPT-5-Mini model support** — Extension now uses `gpt-5-mini` via `vscode.lm.selectChatModels({ vendor: 'copilot', family: 'gpt-5-mini' })` for faster, more efficient responses
- **Phase re-iteration** — All agents support `overwrite: true`, enabling iterative document refinement: agents re-run the current phase and overwrite the document when the user requests changes instead of appending duplicate content
- **Natural-language session entry** — Plain-text `@docu` messages (no slash command) are classified by the LLM, routed to the correct specialist agent, and begin a full iterative document session automatically
- **SetDocument tracking** — Active document path is tracked per conversation so follow-up turns update the same file

### Changed
- `docu.debug.autoStart` default changed to `false` — debug server must be explicitly enabled (security improvement)
- `BrainstormerAgent` now makes real LLM calls via `context.model.sendRequest()` — previously returned static strings
- All agents updated to use typed state accessor functions instead of raw `stateManager.getComponent()` calls

## [0.3.0-rc1] - 2026-02-18
